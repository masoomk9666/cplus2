...
//   return <canvas ref={canvasRef} className="w-full h-full relative bg-transparent" />;
// };
...
//         /* ✅ TRANSPARENT BACKGROUND (SMOG SAFE) */
//         float alphaVal = clamp(length(col), 0.0, 1.0);
...
const Lightning = ({
  ...
  useEffect(() => {
    ...
    /* transparent background (already requested before) */
    gl.enable(gl.BLEND);
    gl.blendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA);
    gl.clearColor(0, 0, 0, 0);
    ...
    const fragmentShaderSource = `
      ...
      void mainImage(out vec4 fragColor, in vec2 fragCoord) {
        ...
        /* 🔒 END POINT FIX (ONLY CHANGE) */
        float anchor = smoothstep(-1.0, 0.2, uv.y);
        uv += anchor * (2.0 * fbm(uv * uSize + 0.8 * iTime * uSpeed) - 1.0);
    ...
    const h = gl.getUniformLocation(program, "uHue");
    const x = gl.getUniformLocation(program, "uXOffset");
    const s = gl.getUniformLocation(program, "uSpeed");
    const i = gl.getUniformLocation(program, "uIntensity");
    const z = gl.getUniformLocation(program, "uSize");
    ...
    const render = () => {
      ...
      gl.uniform1f(h, hue);
      gl.uniform1f(x, xOffset);
      gl.uniform1f(s, speed);
      gl.uniform1f(i, intensity);
      gl.uniform1f(z, size);
      gl.clear(gl.COLOR_BUFFER_BIT);
      gl.drawArrays(gl.TRIANGLES, 0, 6);
      requestAnimationFrame(render);
